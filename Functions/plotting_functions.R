...
.build_exploration_plot <- function(datos, desc, params) {
  ...
  ggplot(
    datos,
    aes(
      x = interaction(Tipo_objeto, Tratamiento),
      y = t_exploracion,
      group = Animal,
      color = Tratamiento, 
      shape = Sex
    ...
    geom_point(
      desc, 
      mapping = aes(x = interaction(Tipo_objeto, Tratamiento), 
                    y = media, group = Tratamiento), 
      color = "black", 
...
.build_DI_plot <- function(datos, desc, params) {
  ...
  ggplot(
    ...
    geom_point(desc, 
               mapping = aes(x = Tratamiento, y = media * 100, group = Tratamiento), 
               color = "black", 
               size = params$mean_size, 
               inherit.aes = F) +
    
    geom_errorbar(desc, 
                  mapping = aes(x = Tratamiento, y = media * 100, ymin = (media - sem) * 100, ymax = (media + sem) * 100, width = params$error_width), 
                  linewidth = params$line_width, 
                  color = "black", 
                  inherit.aes = F) +
...
.build_total_plot <- function(datos, desc, params, y_var, y_label = "Exploration (s)") {
  ...
  ggplot(
    ...
    geom_point(desc, mapping = aes(x = Tratamiento, y = media, group = Tratamiento), color = "black", size = params$mean_size, inherit.aes = F) +
    
    geom_errorbar(desc, mapping = aes(x = Tratamiento, y = media, ymin = media - sem, ymax = media + sem, width = params$error_width), linewidth = params$line_width, color = "black", 
                  inherit.aes = F) +
...
plot_behavior <- function(type,
  ...
  p <- switch(
    ...
    western     = .build_total_plot(datos, desc, params, y_var), 
